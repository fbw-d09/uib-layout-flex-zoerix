...
beforeAll(async () => {
    ...
    await page.goto('file://' + path.resolve('challenges/05/index.html'));
}, 30000);
...
describe("Challenge 05", () => {
    ...
    it("`divs` should be display in the center of the container", async () => {
        const divsOrder = await page.$$eval('*', el => Array.from(el).map(e => getComputedStyle(e).getPropertyValue('justify-content')));
